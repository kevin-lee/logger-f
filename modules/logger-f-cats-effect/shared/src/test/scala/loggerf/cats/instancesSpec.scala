...
object instancesSpec extends Properties {
  override def tests: List[Test] = List(
    ...
    property("test Log.logPure(F[Option[A]])(message, ignoreA)", testLogPureFOptionAIgnoreASome),
    ...
    property("test Log.logPure(F[Either[A, B]])(ignoreA, message)", testLogPureFEitherABIgnoreALeft),
    ...
    property("test Log.logPure(F[Either[A, B]])(ignoreA, message)", testLogPureFEitherABIgnoreARight),
  )
  ...
  private def testLogPureFA: Property = for {
    debugMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("debugMsg")
    infoMsg  <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("infoMsg")
    warnMsg  <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("warnMsg")
    errorMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("errorMsg")
  ...
  private def testLogPureFOptionA: Property = for {
    logMsg     <- Gen.string(Gen.unicode, Range.linear(1, 20)).option.log("logMsg")
    ifEmptyMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).map("[Empty] " + _).log("ifEmptyMsg")
  } yield {
  ...
  private def testLogPureFOptionAIgnoreEmpty: Property = for {
    logMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).option.log("logMsg")
  } yield {
  ...
  private def testLogPureFOptionAIgnoreSome: Property = for {
    logMsg     <- Gen.string(Gen.unicode, Range.linear(1, 20)).option.log("logMsg")
    ifEmptyMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).map("[Empty] " + _).log("ifEmptyMsg")
  } yield {
  ...
  private def testLogPureFOptionAIgnoreASome: Property = for {
    logMsg     <- Gen.string(Gen.unicode, Range.linear(1, 20)).option.log("logMsg")
    ifEmptyMsg <- Gen.string(Gen.unicode, Range.linear(1, 20)).map("[Empty] " + _).log("ifEmptyMsg")
  } yield {
    ...
    def runLog[F[*]: FxCtor: Monad](oa: Option[String]): F[Option[Unit]] =
      (for {
        _ <- Log[F].logPure(pureOf(oa))(error(ifEmptyMsg), ignoreA)
        _ <- Log[F].logPure(pureOf(oa))(error(ifEmptyMsg), ignoreA)
        _ <- Log[F].logPure(pureOf(oa))(error(ifEmptyMsg), ignoreA)
        _ <- Log[F].logPure(pureOf(oa))(error(ifEmptyMsg), ignoreA)
      } yield ().some)
  ...
  private def testLogPureFEitherAB: Property = for {
    rightInt   <- Gen.int(Range.linear(Int.MinValue, Int.MaxValue)).log("rightInt")
    leftString <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("leftString")
    isRight    <- Gen.boolean.log("isRight")
  } yield {
  ...
  private def testLogPureFEitherABIgnoreLeft: Property = for {
    rightInt   <- Gen.int(Range.linear(Int.MinValue, Int.MaxValue)).log("rightInt")
    leftString <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("leftString")
    isRight    <- Gen.boolean.log("isRight")
  } yield {
  ...
  private def testLogPureFEitherABIgnoreALeft: Property = for {
    rightInt   <- Gen.int(Range.linear(Int.MinValue, Int.MaxValue)).log("rightInt")
    leftString <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("leftString")
    isRight    <- Gen.boolean.log("isRight")
  } yield {
    ...
    def runLog[F[*]: FxCtor: Monad](eab: Either[String, Int]): F[Either[String, Unit]] = for {
      _ <- Log[F].logPure(pureOf(eab))(ignoreA, b => debug(b.toString))
      _ <- Log[F].logPure(pureOf(eab))(ignoreA, b => info(b.toString))
      _ <- Log[F].logPure(pureOf(eab))(ignoreA, b => warn(b.toString))
      _ <- Log[F].logPure(pureOf(eab))(ignoreA, b => error(b.toString))
    } yield ().asRight[String]
  ...
  private def testLogPureFEitherABIgnoreRight: Property = for {
    rightInt   <- Gen.int(Range.linear(Int.MinValue, Int.MaxValue)).log("rightInt")
    leftString <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("leftString")
    isRight    <- Gen.boolean.log("isRight")
  } yield {
  ...
  private def testLogPureFEitherABIgnoreARight: Property = for {
    rightInt   <- Gen.int(Range.linear(Int.MinValue, Int.MaxValue)).log("rightInt")
    leftString <- Gen.string(Gen.unicode, Range.linear(1, 20)).log("leftString")
    isRight    <- Gen.boolean.log("isRight")
  } yield {
    ...
    def runLog[F[*]: FxCtor: Monad](eab: Either[String, Int]): F[Either[String, Unit]] = for {
      _ <- Log[F].logPure(pureOf(eab))(error, ignoreA)
      _ <- Log[F].logPure(pureOf(eab))(error, ignoreA)
      _ <- Log[F].logPure(pureOf(eab))(error, ignoreA)
      _ <- Log[F].logPure(pureOf(eab))(error, ignoreA)
    } yield ().asRight[String]
