...
object syntax extends syntax {
  final class LogFASyntax[F[_], A](val fa: F[A]) extends AnyVal {
    ...
    def log(toLeveledMessage: A => LeveledMessage with NotIgnorable)(implicit L: Log[F]): F[A] =
      syntax.log(fa)(toLeveledMessage)

    def logPure(toLeveledMessage: A => LeveledMessage with NotIgnorable)(implicit L: Log[F]): F[A] =
